...
public partial class UnloadingSelectorPage : ContentPage
{
	#region Свойства
	...
	public UnloadingSelectorPage(IBatchService batchService, IBallonService ballonService)
	{
		InitializeComponent();
	...
	private void Button_select_active_unloading_batces_Clicked(object sender, EventArgs e)
	{
		ShowBatchCollectionPopup("Отгрузка");
	}
	...
	private async void ButtonStartNewUnloading_Clicked(object sender, EventArgs e)
	{
		await Shell.Current.GoToAsync($"{nameof(LoadingPage)}?Title=Отгрузка", true);
	}
}